...
from src.database.models import SamplingDecision, AnalysisStatus, RevisitStatus, Frame
...
def test_final_analyzer_no_expert(config, db, prep_story):
    story, frames = prep_story
    db.save_initial_analysis(story.story_id, "model", "sum", "vis", 0.9, SamplingDecision.SUFFICIENT)
...
def test_report_generator_text(db, prep_story):
    story, _ = prep_story
    db.save_initial_analysis(story.story_id, "model", "sum", "vis", 0.9, SamplingDecision.SUFFICIENT)
    db.save_final_analysis(story.story_id, "model", "type", "Desc", "info", "text", "people", "objs", "ctx", 0.9)
